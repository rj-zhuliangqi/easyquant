...
class DashboardService:
    def get_latest_rankings(self, session: Session, sector_type: str, limit: int = 10, metric: str = "net_strength") -> dict:
        ...
        leaders = [self._snapshot_to_dict(row, metric) for row in ranked_rows[:limit]]
        laggards = [self._snapshot_to_dict(row, metric) for row in list(reversed(ranked_rows[-limit:]))]
        return {
    ...
    def get_comparison_series(
        ...
        if granularity == "day":
            ...
            ranked_sector_names = [
                row.sector_name
                for row in sorted(latest_rows, key=lambda row: self._metric_value(row, metric), reverse=True)[:limit]
            ]
            ...
            rows = self._rows_for_lookback(session, sector_type, lookback_days)
    ...
    def get_sector_history(
        ...
        rows = [row for row in self._rows_for_lookback(session, sector_type, lookback_days) if row.sector_name == sector_name]
        return self._build_series(rows, sector_name=sector_name, metric=metric, granularity=granularity)

    def get_sector_snapshot(self, session: Session, sector_type: str, sector_name: str, metric: str = "net_strength") -> dict | None:
        row = session.scalar(
    ...
    def get_alerts(self, session: Session, sector_type: str, metric: str, limit: int = 10) -> dict:
        timestamps = self._latest_two_timestamps(session, sector_type)
        if len(timestamps) < 2:
        ...
        return {"updated_at": timestamps[0].isoformat(), "metric": metric, "items": items[:limit]}
    ...
    def _rows_for_lookback(self, session: Session, sector_type: str, lookback_days: int) -> list[FundFlowSnapshot]:
        all_rows = list(
            session.scalars(
                select(FundFlowSnapshot)
    ...
    def _latest_timestamp(self, session: Session, sector_type: str) -> datetime | None:
        return session.scalar(select(func.max(FundFlowSnapshot.captured_at)).where(FundFlowSnapshot.sector_type == sector_type))

    def _latest_two_timestamps(self, session: Session, sector_type: str) -> list[datetime]:
        return list(
            session.scalars(
